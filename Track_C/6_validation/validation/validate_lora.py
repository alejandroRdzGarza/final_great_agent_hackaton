...
# Configuration
LORA_PATH = r"C:\Projects\Great_Hack\Doctor\global_chain_patch\lora_20251116_064822"
...
# Real-time tracker for attack_successful
successful_count = 0
failed_count = 0
total_processed = 0
...
pbar = tqdm(range(len(df)), desc="Processing rows")
for i in pbar:
    try:
        ...
        total_processed += 1
        if result.get('judgment', {}).get('attack_successful', False):
            successful_count += 1
        else:
            failed_count += 1
        
        # Update progress bar with real-time stats
        pct_successful = (successful_count / total_processed * 100) if total_processed > 0 else 0
        pbar.set_postfix({
            'Success': f'{successful_count}/{total_processed} ({pct_successful:.1f}%)',
            'Failed': f'{failed_count}/{total_processed} ({100-pct_successful:.1f}%)'
        })
        
        # Save results after each row (real-time updates)
        with open(output_file, 'w') as f:
